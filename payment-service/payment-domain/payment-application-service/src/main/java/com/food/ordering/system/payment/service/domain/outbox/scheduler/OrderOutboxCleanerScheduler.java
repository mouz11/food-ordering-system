...
public class OrderOutboxCleanerScheduler implements OutboxScheduler {
    ...
    public void processOutboxMessage() {
        Optional<List<OrderOutboxMessage>> orderOutboxMessagesResponses =
                orderOutboxHelper.getOrderOutboxMessageByOutboxStatus(OutboxStatus.COMPLETED);
        if (orderOutboxMessagesResponses.isEmpty() && orderOutboxMessagesResponses.get().size() > 0) {
            List<OrderOutboxMessage> orderOutboxMessages = orderOutboxMessagesResponses.get();

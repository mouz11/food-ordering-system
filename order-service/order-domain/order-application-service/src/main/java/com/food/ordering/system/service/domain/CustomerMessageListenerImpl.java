...
import com.food.ordering.system.service.domain.ports.input.message.listener.customer.CustomerMessageListener;
...
public class CustomerMessageListenerImpl implements CustomerMessageListener {
    ...
    public void customerCreated(CustomerModel customerModel) {
        ...
        if (customer == null) {
            log.error("Customer could not be created in order database with ud: {}", customerModel.getId());
            throw new OrderDomainException("Customer could not be created in order database with ud: " +
                    customerModel.getId());
        }
        log.info("Customer Is Created in order database with id: {}", customer.getFirstname());
    }
}
